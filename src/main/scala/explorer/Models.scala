...
object Models {
  ...
  case class ExplorerOutputItems(items: Seq[OutputInfo], total: Long)
  case class OutputItems(items: Seq[Output], total: Long)
  ...
  object OutputItems extends ExplorerConversion[ExplorerOutputItems, OutputItems]{
    override def fromExplorer(exp: ExplorerOutputItems): OutputItems = {
      OutputItems(exp.items.map(Output.fromExplorer), exp.total)
    }
  }
